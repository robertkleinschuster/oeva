...
export const FeedStatus = ({feed}: {feed: TransitFeed}) => {
    return <p>
        ...
        {feed.status === TransitFeedStatus.IMPORTING ? <>Importieren: {feed.current_step} ({feed.imported.length + 1} / {feed.files?.size})</> : null}
        {feed.status === TransitFeedStatus.PROCESSING ? <>Verarbeiten: {feed.current_step}</> : null}
